...
| **Does a household hold a goods stock between operations?** `GoodsUnit` is one line per sector and §9.5 has seven, so the answer is worth 7 slots per household — **84.0 MB**, more than the whole holdings table costs today. Nothing in §9 says. §3.4's ten-slot block holds the enumerated tail only if the answer is no, and then with nothing spare | measured by `aurora-tools sizing` (W7.6) | **M1** re-derives the block capacity with the schema; the consumption question is **M6**'s |
