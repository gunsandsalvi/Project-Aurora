...
| **The block model needs a rule bounding what a class member may hold, and has none.** ~~Does a household hold a goods stock?~~ **Derived: no** — §9.4 has no Consumption position and no larder, so one goods line is live at a time, provided position 14 pairs each purchase with its move. What remains is that the tail is *unbounded*: nothing stops a household holding fifty equity lines, so a fixed block plus "exhaustion is a halt" is a halt waiting at every capacity. The enumerated tail is 11 against 10 declared | measured by `aurora-tools sizing` (W7.6) | **M6** states the portfolio rule (a household holds beyond *n* lines only through a fund); **M1** derives the capacity from it |
| **Tenancies, pension claims and insurance claims have no instrument type.** §5.2's census counts 250,000 live tenancies and §8.4's liability-matched institution issues claims of both kinds, against eight opening types that include none of them | found writing W7.6's enumeration | **M9**, which adds instrument types; each is at least one more household slot at 12.0 MB per slot |
| **§12's targets need two numbers, not one.** A clearing position appends no journal row, so it is not in the operation count — but position 13 sorts ~3.34 M submissions across 1,266 lines, about **38.0 M comparisons a tick, 11.7× the whole operation count**. §12.2 budgets 96.5 ns per exchange and says nothing about a comparison | measured by `aurora-tools workload` (W7.4) | **M0** states it; **G0** decomposes its rescope thresholds against both numbers |
| **Which bucket does each obligation type pay in?** §9.4's position 4 walk takes "the standing bucket first", so buckets exist, but nothing says whether rents and amortisation are weekly or monthly. §3.4's 926,246 implies weekly for all of them; a four-tick bucket makes the position 502,500, 46% less | measured by `aurora-tools workload` (W7.4) | **M6**, which owns the obligation walk |
| **Nothing says who is refused when credit is rationed.** `credit.rationing_rule` has no source: §6.3 covers rounding, §9.2 allocates against settled balances, §9.3 clears. The behaviour run reaches it at tick 13 and gets away with it only because there is one borrower. **Rationing is where a credit model's behaviour lives** | measured by `aurora-tools registry-cost` (W7.7) | **M3**, which builds credit |
